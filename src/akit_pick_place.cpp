...
bool akit_pick_place::rotateGripper(std::string object_id)
{  // needs adjusting !!
...
bool akit_pick_place::executeAxisCartesianMotion(bool direction, double cartesian_distance, char axis)
{
  ...
  transform_listener.transformPose(
      EEF_PARENT_LINK, ros::Time(0), pose_in_base_frame, BASE_LINK, pose_in_eef_parent_frame);
  ...
  transform_listener.transformPose(
      BASE_LINK, ros::Time(0), pose_in_eef_parent_frame, EEF_PARENT_LINK, pose_in_base_frame);
...
bool akit_pick_place::planAndExecuteJointGoals(std::string group,
                                               std::vector<double> joint_states,
                                               bool add_to_current_joint_states)
{
  ...
  else if (group == EEF_GROUP)
  {  // for robots where the rotator joint is in the gripper group not in the arm, needs further testing whether to
     // remove or leave
...
bool akit_pick_place::pick(std::string object_id, bool new_grasp_generation)
{  // temp bool until akit is finished
  ...
  if (!side_grasps)
  {  // rotating gripper to adjust with different orientations (only works with top grasping)
                                           ...
                                           shape_msgs::SolidPrimitive shape)
{
  ...
  {  // fix for cuboids
    ...
    i_marker.scale.x = i_marker.scale.y = i_marker.scale.z =
        shape.dimensions[0] + 0.01;  // cannot be same size as collision object, won't return feedback
  }
