...
akit_pick_place::akit_pick_place()
{
  ...
  e1_execute_traj_client =
      nh.serviceClient<moveit_msgs::ExecuteKnownTrajectory>("/e1_moveit_interface/execute_kinematic_path");

  e1_clear_traj_client = nh.serviceClient<std_srvs::Empty>("/e1_moveit_interface/clear_trajectory_list");

  e1_joint_states_subscriber =
      nh.subscribe("/e1_interface/joint_states", 1000, &akit_pick_place::jointStatesCallback, this);
}
...
void akit_pick_place::generateGrasps(geometry_msgs::Pose block_pose_,
                                     double block_size_,
                                     bool sideGrasps,
                                     bool visualize)
{
  ...
  transform_listener.waitForTransform(
      BASE_LINK, WORLD_FRAME, ros::Time::now(), ros::Duration(0.1));  // avoid time difference exception
  ...
  if (!sideGrasps)
  {
    ...
    double line_length =
        sqrt(pow(box_in_chassis_frame.pose.position.x, 2) + pow(box_in_chassis_frame.pose.position.y, 2));
    ...
    for (double i = step_size; i <= covered_distance; i += step_size)
    {
      ...
      grasp_pose.position.z = GRIPPER_LENGTH + box_in_chassis_frame.pose.position.z +
                              (block_size_ / 2);  // divide by 2 --> center of gravity of cube
  ...
  {  // side grasps
    ...
    double blockHypotenuse =
        sqrt(pow(block_size_, 2) + pow(block_size_, 2) + pow(block_size_, 2));  // internal diagonal of cube
...
void akit_pick_place::generateGrasps(geometry_msgs::Pose cuboid_pose_,
                                     double cuboid_x_,
                                     double cuboid_y_,
                                     double cuboid_z_,
                                     bool sideGrasps,
                                     bool visualize)
{
  ...
  tf::Quaternion qq(
      cuboid_pose_.orientation.x, cuboid_pose_.orientation.y, cuboid_pose_.orientation.z, cuboid_pose_.orientation.w);
  tf::Matrix3x3 m(qq);
  ...
  transform_listener.transformPose(
      BASE_LINK, ros::Time(0), cuboid_in_world_frame, WORLD_FRAME, cuboid_in_chassis_frame);
  ...
  if (!sideGrasps)
  {
    // calculate length between base frame origin to object frame
    double line_length =
        sqrt(pow(cuboid_in_chassis_frame.pose.position.x, 2) + pow(cuboid_in_chassis_frame.pose.position.y, 2));
    double number_of_steps = 10.0;
...
void akit_pick_place::generateGrasps(geometry_msgs::Pose cylinder_pose_,
                                     ...
                                     bool visualize)
{
  ...
  // calculate roll,pitch,yaw of the object relative to the world frame for test variable (top grasping)
  tf::Quaternion qq(cylinder_pose_.orientation.x,
                    cylinder_pose_.orientation.y,
                    cylinder_pose_.orientation.z,
                    cylinder_pose_.orientation.w);
  tf::Matrix3x3 m(qq);
  double roll_, pitch_, yaw_;
  m.getRPY(roll_, pitch_, yaw_);
  ...
  transform_listener.waitForTransform(
      BASE_LINK, WORLD_FRAME, ros::Time::now(), ros::Duration(0.1));  // avoid time difference exception
  transform_listener.transformPose(
      BASE_LINK, ros::Time(0), cylinder_in_world_frame, WORLD_FRAME, cylinder_in_chassis_frame);
  ...
  if (!sideGrasps)
  {
    // calculate length between base frame origin to object frame
    double line_length =
        sqrt(pow(cylinder_in_chassis_frame.pose.position.x, 2) + pow(cylinder_in_chassis_frame.pose.position.y, 2));
    double number_of_steps = 10.0;
    ...
    for (double i = step_size; i <= covered_distance; i += step_size)
    {
      ...
      if (std::abs(test) >= sin(M_PI / 4))
      {
        grasp_pose.position.z =
            GRIPPER_LENGTH + cylinder_in_chassis_frame.pose.position.z + (cylinderInternalDiagonal / 2);
      }
      else if (std::abs(test) <= sin(M_PI / 4))
      {
...
bool akit_pick_place::rotateGripper(moveit_msgs::CollisionObject object_)
{  // needs adjusting !!
  ...
  transform_listener.transformPose(
      GRIPPER_FRAME, ros::Time(0), object_in_world_frame, WORLD_FRAME, object_in_gripper_frame);

  // get roll, pitch, yaw between object frame and gripper frame
  tf::Quaternion qq(object_in_gripper_frame.pose.orientation.x,
                    object_in_gripper_frame.pose.orientation.y,
                    object_in_gripper_frame.pose.orientation.z,
                    object_in_gripper_frame.pose.orientation.w);
...
bool akit_pick_place::executeAxisCartesianMotion(bool direction, double cartesian_distance, char axis)
{
  ...
  pose_in_base_link_frame =
      getPositionFK_msg.response.pose_stamped.back();   // eef == quickcoupler is the last link in the chain
  ...
  transform_listener.transformPose(
      EEF_PARENT_LINK, ros::Time(0), pose_in_base_link_frame, BASE_LINK, pose_in_eef_frame);

  if (!direction)
  {  // downwards cartesian motion in quickcoupler frame
    switch (axis)
    {
      case 'x':
  ...
  {  // upwards cartesian motion in quickcoupler frame
    switch (axis)
    {
      case 'x':
        pose_in_eef_frame.pose.position.x += cartesian_distance;
  ...
  transform_listener.transformPose(
      BASE_LINK, ros::Time(0), pose_in_eef_frame, EEF_PARENT_LINK, pose_in_base_link_frame);
...
bool akit_pick_place::planAndExecuteCartesianGoals(std::vector<geometry_msgs::Pose> poses, std::string pose)
{
  ...
  for (int i = 0; i < poses.size(); i++)
  {
    ...
    {
      ROS_ERROR_STREAM("motion planning to " << pose << " position " << count << " failed on error code = "
                                             << e1_go_to_srv.response.error.val);
...
bool akit_pick_place::attachCollisionObject(moveit_msgs::CollisionObject collisionObject)
{
  ...
  attached_collision_object.object.operation =
      attached_collision_object.object.REMOVE;  // operation bit is REMOVE from world
  ...
  attached_collision_object.object.operation = attached_collision_object.object.ADD;  // operation bit is ADD to
                                                                                      // attached
...
bool akit_pick_place::detachCollisionObject(moveit_msgs::CollisionObject collisionObject)
{
  ...
  // get robot state from service call --> returns  CURRENT attached collision object info so that object is returned to
  // env at correct pose
  ...
  detached_collision_object.object.header.frame_id =
      get_planning_scene_srv.response.scene.robot_state.attached_collision_objects[0].object.header.frame_id;
  detached_collision_object.object.id =
      get_planning_scene_srv.response.scene.robot_state.attached_collision_objects[0].object.id;
  detached_collision_object.link_name =
      get_planning_scene_srv.response.scene.robot_state.attached_collision_objects[0].link_name;
  detached_collision_object.object.primitives.push_back(
      get_planning_scene_srv.response.scene.robot_state.attached_collision_objects[0].object.primitives[0]);
  detached_collision_object.object.primitive_poses.push_back(
      get_planning_scene_srv.response.scene.robot_state.attached_collision_objects[0].object.primitive_poses[0]);
  ...
  detached_collision_object.object.operation =
      detached_collision_object.object.REMOVE;  // operation bit is REMOVE from attached
  ...
  detached_collision_object.object.operation =
      detached_collision_object.object.ADD;  // operation bit is ADD to world again
  planning_scene.world.collision_objects.clear();
  planning_scene.world.collision_objects.push_back(
      detached_collision_object.object);  // spawns the object relative to gripper frame !! check
  planning_scene.is_diff = true;
...
bool akit_pick_place::pick(moveit_msgs::CollisionObject object_)
{
  ...
  {  // if grasp poses are entered separatly from blender(needed if object is not a box,cuboid,cylinder!)
...
bool akit_pick_place::place(moveit_msgs::CollisionObject object_)
{
  ...
  {  // if place poses are entered separatly from blender(needed if object is not a box,cuboid,cylinder!)
  ...
  // cartesian motion downwards
  if (!this->executeAxisCartesianMotion(DOWN, GRIPPER_JAW_LENGTH, 'z'))
  {  // experiment and change gripper_jaw_length --> drop the object rather than place it
    ROS_ERROR("Failed to execute downwards cartesian motion");
    return false;
    exit(1);
  }
...
CollisionObjectsMap akit_pick_place::getSceneCollisionObjects()
{
  ...
  std::vector<moveit_msgs::CollisionObject> scene_objects =
      get_planning_scene_msg.response.scene.world.collision_objects;
...
AttachedCollisionObjectsMap akit_pick_place::getAttachedCollisionObjects()
{
  ...
  std::vector<moveit_msgs::AttachedCollisionObject> attached_objects =
      get_planning_scene_msg.response.scene.robot_state.attached_collision_objects;
                                           ...
                                           shape_msgs::SolidPrimitive shape)
{
  ...
  {  // fix for cuboids
    ...
    i_marker.scale.x = i_marker.scale.y = i_marker.scale.z =
        shape.dimensions[0] + tolerance;  // cannot be same size as collision object, won't return feedback
  }
  ...
  // add the interactive marker to our collection & tell the server to call processFeedback() when feedback arrives for
  // it
...
bool akit_pick_place::interactive_pick_place(std::vector<geometry_msgs::Pose> place_positions)
{
  ...
  while (ros::ok() && count < place_positions.size())
  {
    ...
    if (msgReceived)
    {
      ...
      for (it = collision_objects_map.begin(); it != collision_objects_map.end(); ++it)
      {
        if (interactive_name == it->first)
        {
          ...
          {
            this->generateGrasps(
                interactive_pose, it->second.primitives[0].dimensions[0], it->second.primitives[0].dimensions[1]);
    ...
    for (a_it = attached_collision_objects_map.begin(); a_it != attached_collision_objects_map.end(); ++a_it)
    {
      ...
      {
        this->generateGrasps(place_positions[count],
                             a_it->second.object.primitives[0].dimensions[0],
                             a_it->second.object.primitives[0].dimensions[1]);
...
bool akit_pick_place::attachTool(std::string tool_frame_id)
{  // change to tool frame id and remove if conditions!
  ...
  transform_listener.waitForTransform(
      BASE_LINK, tool_frame_id, ros::Time::now(), ros::Duration(0.1));  // avoid time difference exception
  transform_listener.transformPose(
      BASE_LINK, ros::Time(0), initial_pose_tool_frame, tool_frame_id, initial_pose_base_frame);
