...
int main(int argc, char** argv)
{
  ...
  ros::ServiceClient plannnig_scene_diff_client =
  ...
  const robot_state::JointModelGroup* joint_model_group =
      akitGroup.getCurrentState()->getJointModelGroup("e1_complete");
  ...
  listener.waitForTransform(
      "chassis", "bucket_raedlinger", ros::Time::now(), ros::Duration(0.1));  // avoid time difference exception
  ...
  listener.waitForTransform(
      "quickcoupler", "chassis", ros::Time::now(), ros::Duration(0.1));  // avoid time difference exception
  listener.transformPose(
      "quickcoupler", ros::Time(0), marker_pose_in_world_frame, "chassis", marker_pose_in_quickcoupler_frame);
  ...
  listener.waitForTransform(
      "chassis", "quickcoupler", ros::Time::now(), ros::Duration(0.1));  // avoid time difference exception
  listener.transformPose(
      "chassis", ros::Time(0), marker_pose_in_quickcoupler_frame, "quickcoupler", marker_pose_in_world_frame);
